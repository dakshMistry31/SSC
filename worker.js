...
var FALLBACK_MODEL = "gemini-1.5-flash";
...
function uniqueQuestions(questions) {
  ...
  for (const q of questions) {
    const key = cleanString(q.question, 800)
      .toLowerCase()
      .replace(/\s+/g, " ");
    ...
    const options = Array.isArray(q.options)
      ? q.options.map((x) => cleanString(x, 300))
      : [];
    ...
    if (!cleanString(q.explanation, 1200)) {
      continue;
    }
...
__name(uniqueQuestions, "uniqueQuestions");
...
async function verifyFirebaseToken(token, env) {
  ...
  if (!account?.localId || account.disabled) {
    throw new Error("Firebase account is not valid.");
  }
...
function buildPrompt({
  ...
  const weakTopics = (Array.isArray(performance?.weakTopics)
...
Weak topics: ${
    weakTopics.length ? weakTopics.join(", ") : "none recorded"
  }
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        response = await fetch(
          `${model.url}?key=${encodeURIComponent(env.GEMINI_API_KEY)}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json"
            ...
            body: JSON.stringify({
              systemInstruction: {
                parts: [
                  {
                    text: "You output only structured JSON for a secure exam-question API. Follow the requested schema exactly. Keep explanations concise so the complete test can be returned quickly."
                  }
                ]
              },
              contents: [
        ...
        if (error?.name === "AbortError") {
          lastError =
            "AI generation timed out. Please try again with 10 questions for a faster test.";
        } else {
      ...
      if (!response.ok) {
        ...
        console.log(
          `Gemini ${model.name} attempt ${attempt + 1}/${maxAttempts} failed:`,
          response.status,
          detail
        );
        ...
        if (temporaryError && attempt < maxAttempts - 1) {
      ...
      try {
        ...
        throw new Error("Gemini question data could not be parsed.");
      }
    }

    console.log(
      `Primary/fallback transition: trying next model after ${model.name}`
    );
  }

  throw new Error(
    lastError || "AI generation failed after trying all available models."
  );
}
...
async function handleGenerate(request, env) {
  ...
  if (!subject) {
    return json(
      {
        error: "Invalid CHSL tier or subject."
      },
      400,
      env,
      request
  ...
  const count = Math.min(
    Math.max(Number(body?.count) || 20, 10),
    20
  );
  ...
  if (
    ...
    return json(
      {
        error: "Invalid topic for the selected CHSL subject."
      },
      400,
      env,
      request
  ...
  try {
    ...
    return json(
      {
        error:
          "The AI returned duplicate or incomplete questions. Please generate the test again."
      },
      502,
      env,
      request
